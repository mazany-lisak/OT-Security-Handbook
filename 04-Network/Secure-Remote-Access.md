...
review_cycle: Annual
summary: >-
  Brokered remote-access architecture for OT: identity and MFA before connectivity, VPN selection
  and hardening (WireGuard → IPsec → OpenVPN), jump/bastion hosts, vendor governance, session
  recording and operating without the remote path.
keywords: [remote access, vzdálený přístup, VPN, WireGuard, IPsec, jump server, vendor access, session recording, MFA]
---

> **What this document is.** A source-verified knowledge base on **secure remote access to OT/ICS**, written as an **LLM skill / RAG** reference. It covers *who* may connect from outside the control network and *how* each session is brokered, authenticated, restricted, recorded and torn down. It is the access-layer companion to [Network-Segmentation.md](Network-Segmentation.md) (which defines *where* the boundary sits — the Industrial DMZ) and [Firewall-Design.md](Firewall-Design.md) (which *enforces* the conduit). Remote access is consistently identified by NSA/CISA as one of the highest-risk and most-exploited paths into OT.
>
> **Deliberate scope limits.** Boundary placement and the IDMZ are defined in [Network-Segmentation.md](Network-Segmentation.md); rule enforcement and device selection are in [Firewall-Design.md](Firewall-Design.md); identity lifecycle (joiner/mover/leaver, RBAC, PAM detail) is in [Identity-Management.md](../05-Identity/Identity-Management.md); engineering/programming-protocol behaviour is in [OT-Protocols.md](OT-Protocols.md); zone/conduit and FR theory is in [IEC62443.md](../02-Standards/IEC62443.md).
>
...
5. **Treat the remote session itself as administrative/engineering access** and apply the controls in [OT-Protocols.md](OT-Protocols.md) (program-mode, setpoint and logic-integrity protections).
...
7. **Reference** [Network-Segmentation.md](Network-Segmentation.md), [Firewall-Design.md](Firewall-Design.md), [Identity-Management.md](../05-Identity/Identity-Management.md), [OT-Protocols.md](OT-Protocols.md), [IEC62443.md](../02-Standards/IEC62443.md) instead of restating them.
...
Covers remote-access architecture, identity and authentication, VPN selection and hardening, jump/bastion hosts, vendor/third-party governance, least-privilege and just-in-time access, controller protection during sessions, emerging access models, session monitoring/auditing, and resilience. **Out of scope (referenced):** boundary/IDMZ design ([Network-Segmentation.md](Network-Segmentation.md)), firewall enforcement ([Firewall-Design.md](Firewall-Design.md)), identity lifecycle ([Identity-Management.md](../05-Identity/Identity-Management.md)), protocols ([OT-Protocols.md](OT-Protocols.md)).
...
- **The IDMZ does the brokering** (see [Network-Segmentation.md](Network-Segmentation.md) §8 and [ISA95.md](../02-Standards/ISA95.md) Industrial DMZ patterns); the firewall enforces the conduit (see [Firewall-Design.md](Firewall-Design.md) §7 vendor-access placement).

# 6. Identity and Authentication

- **Named, individual accounts.** No shared/generic/vendor-pool accounts; every action attributable. (Lifecycle in [Identity-Management.md](../05-Identity/Identity-Management.md).)
...
- **Centralized AAA** (RADIUS/TACACS+ → IdP) rather than per-device or per-application MFA settings, to close configuration gaps. (See [OT-Protocols.md](OT-Protocols.md) for RADIUS/TACACS+/LDAPS transport.)
...
**Technology preference (per the IACS edge-gateway guidance, e.g. ICR-2734 — see [ICR-2734.md](../08-Technologies/Advantech/ICR-Series/ICR-2734.md)):** **WireGuard** (modern, small attack surface, key-based) → **IPsec (IKEv2)** → **OpenVPN** (legacy interoperability); **GRE only inside IPsec**. Prefer **cryptographic credentials over passwords** for tunnel authentication.
...
- **Filter to known source IPs**; apply geo-blocking where appropriate; default-deny everything else (see [Firewall-Design.md](Firewall-Design.md)).
...
Third-party access is the **single highest-risk remote-access category** and a NIS2/Czech-Act supply-chain concern (see [NIS2.md](../01-Legislation/NIS2.md) → *Article 21(2)(d) and 22 — Supply Chain Security*; [Czech-Cybersecurity-Act.md](../01-Legislation/Czech-Cybersecurity-Act.md) → *Domain 11 — Supply chain*). Govern it explicitly:
...
A remote session is **administrative/engineering access** and inherits the controller-level risks in [OT-Protocols.md](OT-Protocols.md). During any remote engineering access:

- **Prevent controllers from remaining in remote *program* mode during operation** where possible; use the platform's protection (e.g. Siemens S7-1500 protection levels / PUT-GET off; Schneider M580 RUN/STOP authorization bit and Memory Protect — see [OT-Protocols.md](OT-Protocols.md) platform chapters).
...
- Correlate VPN, jump-host, AD/identity, firewall and ICS-IDS logs (see [Network-Segmentation.md](Network-Segmentation.md) §13, [Firewall-Design.md](Firewall-Design.md) §10–11).

# 13. Resilience — Operating Without the Remote Path

Assume the remote path can be compromised or must be severed. Maintain and regularly exercise the ability to **operate the process manually**, to **isolate OT from a compromised IT/remote path** (workarounds, manual controls), and to **restore** OT devices/services from tested backups. Define roles for emergency disconnection. (See [ICR-2734.md](../08-Technologies/Advantech/ICR-Series/ICR-2734.md), [Disaster-Recovery.md](../09-Operations/Disaster-Recovery.md).)

# 14. Regulatory Verification & Audit Mapping

| Remote-access concern | IEC 62443 ([IEC62443.md](../02-Standards/IEC62443.md)) | NIS2 ([NIS2.md](../01-Legislation/NIS2.md)) | Czech Act 264/2025 ([Czech-Cybersecurity-Act.md](../01-Legislation/Czech-Cybersecurity-Act.md)) |
|---|---|---|---|
...
- Apply controller protections during sessions (program-mode, setpoints, logic integrity) per [OT-Protocols.md](OT-Protocols.md).
- Log and record everything to the SIEM; preserve manual-operation resilience.
- Map controls to IEC 62443 FR1/FR2/FR5/FR6/FR7, NIS2 Art. 21/22/23, and Czech-Act Domains 4/5/7/11/12; reference [Network-Segmentation.md](Network-Segmentation.md), [Firewall-Design.md](Firewall-Design.md), [Identity-Management.md](../05-Identity/Identity-Management.md), [OT-Protocols.md](OT-Protocols.md), [IEC62443.md](../02-Standards/IEC62443.md) rather than restating them.
...
- [Network-Segmentation.md](Network-Segmentation.md) — IDMZ and remote/vendor-access boundary placement (§8, §9, §10.3).
- [Firewall-Design.md](Firewall-Design.md) — vendor-access gateway placement and enforcement (§7), device selection (§6).
- [Identity-Management.md](../05-Identity/Identity-Management.md) — identity lifecycle, RBAC, MFA, PAM detail.
- [OT-Protocols.md](OT-Protocols.md) — engineering/programming traffic as administrative traffic; controller protections; platform specifics (S7-1500, M580, Twido, X20).
- [IEC62443.md](../02-Standards/IEC62443.md) — FR1 (IAC), FR2 (UC), FR5 (RDF), FR6/FR7, conduits, Defense in Depth, 62443-2-4 supplier requirements.
- [NIS2.md](../01-Legislation/NIS2.md) — Article 21 (access control/MFA), Article 21(2)(d)/22 (supply chain), Article 23 (incident reporting).
- [Czech-Cybersecurity-Act.md](../01-Legislation/Czech-Cybersecurity-Act.md) — Zákon 264/2025 Sb.; Domain 4 (IAM), 5 (segmentation), 7 (logging/monitoring), 11 (supply chain), 12 (business continuity).
- [ICR-2734.md](../08-Technologies/Advantech/ICR-Series/ICR-2734.md) — WireGuard/IPsec edge VPN and secure remote maintenance reference.
- [Disaster-Recovery.md](../09-Operations/Disaster-Recovery.md), [Monitoring.md](../09-Operations/Monitoring.md), [Logging.md](../09-Operations/Logging.md).
...
| 1.1.0 | 2026-07-07 | Corpus restructure: canonical YAML front matter (id, layer, summary, keywords, language); links converted to layer-relative paths per the numbered directory tree; dangling targets remapped; LF line endings; migrated from skill-style front matter; ICR-2734 sub-document references consolidated to ICR-2734.md |
